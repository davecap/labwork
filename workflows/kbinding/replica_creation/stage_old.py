...
def main():    
    ...
    parser.add_option("-o", "--output-pdb", dest="output_pdb", default="aligned", help="Output PDB [default: %default]")
    ...
    if args[0].lower().endswith('.dcd'):
        ...
        tcl = dcd_stage_tcl(staging_psf=options.psf_file, staging_dcd=args[0], starting_pdb=args[1], align_selection=options.align_selection, target_selection=options.target_selection, output_pdb=options.output_pdb)   
    elif args[0].lower().endswith('.pdb'):
        tcl = pdb_stage_tcl(args[0], starting_pdb=args[1], align_selection=options.align_selection, target_selection=options.target_selection, output_pdb=options.output_pdb)
    else:
        parser.error("Invalid input file, must be a PDB or DCD file")
    
    run_VMD(tcl, options.verbose)

def dcd_stage_tcl(staging_psf, staging_dcd, starting_pdb, align_selection, target_selection, output_pdb="staged.pdb"):
...
for {set frame 0} {$$frame < $$num_steps} {incr frame} {
    ...
    $$sel1all writepdb "${output_pdb}_$$frame.pdb"
}
quit
"""
